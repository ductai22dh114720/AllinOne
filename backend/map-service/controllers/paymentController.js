...
const moment = require("moment"); // Đặt moment ở phạm vi toàn cục để dễ dùng
...
// SỬA: HÀM SORT OBJECT THEO CHUẨN VNPAY DEMO (SẮP XẾP KEY VÀ ENCODE VALUE)
const sortObject = (obj) => {
  ...
  for (const key of keys) {
    // Ép kiểu sang chuỗi và ENCODE VALUE TRONG HÀM SORT
...
exports.createTopupPayment = async (req, res) => {
  try {
    ...
    // FIX: BỎ toLocaleString để tránh ký tự '.' gây lỗi chữ ký
    vnp_Params["vnp_OrderInfo"] = `Nap tien vao vi ${amount} VND`;
    ...
    // Sắp xếp params VÀ ENCODE VALUE
    vnp_Params = sortObject(vnp_Params);

    // SỬA: Dùng encode: false vì value đã được encode trong sortObject
    ...
    // QUAN TRỌNG: URL gửi đi không được encode lần nữa
    const paymentUrl =
...
exports.vnpayIpn = async (req, res) => {
  try {
    ...
    // SỬ DỤNG HÀM sortObject MỚI (encode value)
    vnp_Params = sortObject(vnp_Params);
    // Dùng encode: false
    const signData = qs.stringify(vnp_Params, { encode: false });
    const hmac = crypto.createHmac("sha512", vnp_HashSecret);
...
// @desc    Thanh toán dịch vụ bằng ví (Giữ nguyên)
...
exports.payService = async (req, res) => {
  try {
    ...
    // ... (logic thanh toán ví giữ nguyên) ...
...
exports.payServiceWithVNPay = async (req, res) => {
  try {
    ...
    // FIX: Dùng moment cho định dạng ngày tháng
    process.env.TZ = "Asia/Ho_Chi_Minh";
    const date = new Date();
    const createDate = moment(date).format("YYYYMMDDHHmmss");
...
exports.servicePaymentCallback = async (req, res) => {
  try {
    ...
    let vnp_Params = req.query; // Lấy tham số từ URL
    const secureHash = vnp_Params["vnp_SecureHash"];

    delete vnp_Params["vnp_SecureHash"];
    delete vnp_Params["vnp_SecureHashType"];
    delete vnp_Params["serviceId"]; // Loại bỏ tham số riêng
