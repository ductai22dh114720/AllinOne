...
exports.createTopupPayment = async (req, res) => {
  try {
    ...
    process.env.TZ = "Asia/Ho_Chi_Minh";
    const date = new Date();
    // const moment = require("moment"); // Đã require ở đầu file
    const createDate = moment(date).format("YYYYMMDDHHmmss");
    ...
    vnp_Params["vnp_CurrCode"] = "VND";
    vnp_Params["vnp_TxnRef"] = orderId;
    // SỬA: BỎ toLocaleString để tránh ký tự '.' gây lỗi chữ ký
...
exports.vnpayCallback = async (req, res) => {
  try {
    ...
    const signData = qs.stringify(vnp_Params, { encode: false });
    const hmac = crypto.createHmac("sha512", vnp_HashSecret);
    // SỬA: Đảm bảo dùng new Buffer() cho nhất quán với VNPAY Demo
    const signed = hmac.update(new Buffer(signData, "utf-8")).digest("hex");
